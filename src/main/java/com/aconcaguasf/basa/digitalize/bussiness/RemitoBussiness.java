...
//        );
        ...
        if (operacion.getRequerimiento().getTipoRequerimiento().getId().equals(Const.REQ_TIPO_RETIRO_CAJA_CANT)){
            List<Elementos> elementosDeBaja = new ArrayList<>();
            lecturaCamion.getLecturasDetallesList().forEach(elemLectura -> {
                ...
                if (elemento.getClienteEmp_id().equals(operacion.getClienteEmp_id())) {
                    if (elemento.getEstado().equalsIgnoreCase(Const.ELEM_ESTADO_EN_CLIENTE)) {
                        ...
                        remitoProcesoDTO.addElementoAgrupado(
                                new RemitoProcesoDTO.ElementoRemito(elemLectura.getElemento_id(),
                                        ...
                                        Const.ELEM_GRUPO_GUARDA,
                                        codCargaGC.get()));
                    } else if (elemento.getEstado().equalsIgnoreCase(Const.ELEM_ESTADO_EN_CONSULTA)) {
                    ...
                    } else if (elemento.getEstado().equalsIgnoreCase(Const.ELEM_ESTADO_EN_BAJA)) {
                        // Agregar elementos marcados como 'De Baja' a la lista
                        // elementosDeBaja.add(elemento);
                        remitoProcesoDTO.addElementosDeBaja(
                                new RemitoProcesoDTO.ElementoRemito(elemLectura.getElemento_id(),
                                        elemLectura.getCodigoBarras(),
                                        Const.ELEM_COD_GRUPO_DEVOLUCION,
                                        Const.ELEM_GRUPO_DEVOLUCION,
    ...
    public ResponseDTO procesarRemito(Principal principal, RemitoProcesoDTO remitoProceso) throws Exception {
        ...
        if (userJose == 60264 || userJose == 60446 || userJose == 60595 || userJose == 60359  || userJose == 30187) {
            ...
            operaciones.getRelacionOpEl().forEach(elemOp -> {
                ...
                if (remitoProceso.getElementosFaltantesCamion().contains(elemento.getCodigo())) {
                  ...
                  movimientoRemitoRepository.save(movimientoRemito);
                    ...
                    if (Const.ELEM_ESTADO_EN_CLIENTE.equalsIgnoreCase(elemento.getEstado())) {
                        String ingreso = "REMITO: " + remitoProceso.getNumeroRemito() + " - " +
                                DateTimeFormatter.ofPattern("dd/MM/yyyy")
                                        .withZone(ZoneId.systemDefault())
                                        .format(Instant.ofEpochMilli(remitoProceso.getFechaEntregaRemito()));
                        elemento.setIngresoGyC(ingreso);
                    }
                    ...
                    elementosContendor.forEach(elemContenido -> {
                       ...
                       // elementosBussiness.crearElementoHistorico(elemContenido.getCodigo(), elemContenido.getClienteAsp_id()
                        //        , elemContenido.getClienteEmp_id(),userJose, "INGRESO");
                     });
         ...
         //Envio de Correos de Remito
            ...
            operaciones.getRequerimiento().getRemito().setFechaEntrega(operaciones.getFechaEntrega());
            ...
            operaciones.getRequerimiento().setFechaEntrega(operaciones.getFechaEntrega());
    ...
    private static void enviarConGMail(String destinatario, String asunto, StringBuilder cuerpo ,String destinatario1) throws Exception {
      ...
      //  props.put("mail.smtp.starttls.enable", "true"); // Para conectar de manera segura al servidor SMTP
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("Error Cuenta Destino");
        } catch (NoSuchProviderException e) {
            e.printStackTrace();
            System.out.println("Error Servidor de Correos");
