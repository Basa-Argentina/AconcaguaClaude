...
public class OperacionesController {
    ...
    public Page<Operaciones> operacionesFilters(@RequestBody final OperacionesFilter filtro, @AuthenticationPrincipal Principal principal) throws Exception {
     ...
     //   LocalDate localDate = LocalDate.of(2026, 02, 13);
     //   java.util.Date date = java.sql.Date.valueOf(localDate);

        // Sort descending and Search the first 1000 elements
        Pageable pageRequest = new PageRequest(filtro.getPage(), 1000, new Sort(Sort.Direction.DESC, "id"));
        ...
        if (filtro.getNum_requerimiento() != null){
          operaciones = operacionesRepository.findOperacionesByNumReq(filtro.getNum_requerimiento(), pageRequest);
            ...
            // Para que no busque ningun requerimiento Lborello 20260113
        ...
        // Precarga HojaRuta en un solo query por requerimiento único (elimina N+1)
        Map<String, HojaRuta> hdrPorReq = new HashMap<>();
        operaciones.getContent().stream()
                .filter(op -> op.getRequerimiento().getHojaRuta() == null)
                .map(op -> op.getRequerimiento().getNumero())
                .distinct()
                .forEach(numReq -> {
                    HojaRuta hdr = hojaRutaRepository.findHdrByReqNumero(numReq);
                    if (hdr != null) hdrPorReq.put(numReq, hdr);
                });
        operaciones.forEach(operacion -> {
            if(operacion.getRequerimiento().getHojaRuta() == null) {
                operacion.getRequerimiento().setHojaRuta(hdrPorReq.get(operacion.getRequerimiento().getNumero()));
            }
        });
       ...
       /*     } else if (!lecturasA.isEmpty()) {
            ...
            if (lecturasA.isEmpty() && lecturaAC.isEmpty() &&
                ...
                if (operacion.getRequerimiento().getRemito() != null) {
                    ...
                    remito.setFechaEntrega(operacion.getFechaEntrega());
                    remitosRepository.save(remito);
                }

                List<ElementosLite> elementosBatch = new ArrayList<>();
                boolean bajaProcesada = false;
                ...
                for (String ele : lecturaAB) {
                    ...
                    if (tipo.equals(SALIDA)) {
                        ...
                        if (operacion.getRequerimiento().getTipoRequerimiento_id().equals("23")){
                            elementos.setEstado("De Baja");
                            elementos.setRemitoBaja("REMITO ASP: "+operacion.getRequerimiento().getRemito().getNumeroSinPrefijo() +" # " +
                                    operacion.getRequerimiento().getRemito().getFechaEntrega()
                                            .toLocalDate()
                                            .format(DateTimeFormatter.ofPattern("dd/MM/yyyy"))
                                    +" # " +  operacion.getRequerimiento().getRemito().getEmpleadoSolicitante()
                            );
                            operacion.setTipoOperacion_id("88");
                            bajaProcesada = true;
                        }
                        ...
                        if (!elementosContenidos.isEmpty()) {
                            elementosContenidos.forEach(c -> {
                                movimientosBussiness.crearMovimiento(
                                        ...
                                        operacion.getClienteAsp_id(),
                                        operacion.getClienteEmp_id(),
                                        operacion.getRequerimiento().getRemito().getDepositoOrigen_id(), c.getId(),
                                        operacion.getRequerimiento().getRemito_id(),
                                        currentUser.getId(),
                                        operacion.getRequerimiento().getRemito().getNumRequerimiento(),
                                        null
                    ...
                    elementosBatch.add(elementos);
                }
                // Batch save: un solo UPDATE/INSERT por todos los elementos en lugar de uno por elemento
                if (!elementosBatch.isEmpty()) {
                    elementosLiteRepository.save(elementosBatch);
                }
                // Guardar operacion de Baja una sola vez al final del loop
                if (bajaProcesada) {
                    operacionesRepository.save(operacion);
                }
                ...
                // Cache del estado y batch save: un solo INSERT/UPDATE por todos los RelacionOpEl
                EstadoElementoHojaRuta estadoControlado = estadoElementoHojaRutaRepository.findEstadoElementoHojaRutaById(2L);
                if (estadoControlado == null) throw new Exception("Sin hoja de ruta asignada");
                Set<RelacionOpEl> relacionOpElSet = operacion.getRelacionOpEl();
                List<RelacionOpEl> relacionOpElBatch = new ArrayList<>();
                for (RelacionOpEl rel : relacionOpElSet) {
                    rel.setEstadoElementoHojaRuta(estadoControlado);
                    relacionOpElBatch.add(rel);
                }
                if (!relacionOpElBatch.isEmpty()) {
                    relacionOpElRepository.save(relacionOpElBatch);
                }
            }
