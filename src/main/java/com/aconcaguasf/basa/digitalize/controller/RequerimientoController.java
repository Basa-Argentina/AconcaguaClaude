...
public class RequerimientoController {
    ...
    @PreAuthorize("hasAuthority('2')")
    @ResponseBody
    @RequestMapping(value = "/requerimiento/{codigo}/{operacion_id}/verificar_Reqelemento", method = RequestMethod.GET)
    public ResponseEntity<?> checkReqElement(@AuthenticationPrincipal Principal principal,
                                             @PathVariable("codigo") String codigo,
                                             @PathVariable("operacion_id") Long operacion_id) {
        try {
            ElementosOperacionDTO exists = operacionBussiness.checkReqElement(codigo);

            if (exists != null && !exists.getOperacionId().equals(operacion_id)) {
                Map<String, String> errorResponse = new HashMap<>();
                errorResponse.put("error", "Ya existe el elemento en otro requerimiento Activo.");
                return ResponseEntity
                        .status(HttpStatus.BAD_REQUEST)
                        .body(errorResponse);
            } else {
                Map<String, String> successResponse = new HashMap<>();
                successResponse.put("codigo", codigo);
                return ResponseEntity
                        .status(HttpStatus.OK)
            ...
            Map<String, String> errorResponse = new HashMap<>();
            errorResponse.put("error", CustomException.getInstance().getMessage());
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(errorResponse);
        }
