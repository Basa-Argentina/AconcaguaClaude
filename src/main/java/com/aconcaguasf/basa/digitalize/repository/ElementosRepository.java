...
public interface ElementosRepository extends JpaRepository<Elementos, Long> {
    ...
    /*@Query(value = "SELECT        e.codigo, x.operacion_id\n" +
            "FROM            elementos AS e INNER JOIN\n" +
            "                         x_operacion_elemento AS x ON e.id = x.elemento_id\n" +
            "WHERE        (e.codigo = :cod) AND (x.estado = 'Pendiente')", nativeQuery = true)
    Elementos findByReqElement(@Param("cod") String cod); */

    /*@Query(value = "SELECT new com.aconcaguasf.basa.digitalize.dto.ElementosOperacionDTO(e.codigo, x.operacion_id) " +
            "FROM elementos AS e INNER JOIN " +
            "     x_operacion_elemento AS x ON e.id = x.elemento_id " +
            "WHERE e.codigo = :cod AND x.estado = 'Pendiente' ", nativeQuery = true)
    ElementosOperacionDTO findByReqElement(@Param("cod") String cod);*/

   /* @Query(value = "SELECT e.codigo AS codigo, x.operacion_id AS operacionId " +
            "FROM elementos e " +
            "INNER JOIN x_operacion_elemento x ON e.id = x.elemento_id " +
            "WHERE e.codigo = :cod AND x.estado = 'Pendiente'", nativeQuery = true)
    ElementosOperacionProjection findByReqElement(@Param("cod") String cod); */
    ...
    @Query(value = "SELECT e.codigo, x.operacion_id " +
            ...
            "INNER JOIN x_operacion_elemento x ON e.id = x.elemento_id " +
            "WHERE e.codigo = :cod AND x.estado = 'Pendiente'", nativeQuery = true)
    List<Object[]> findByReqElement(@Param("cod") String cod);
 ...
 //   List<Elementos> findByCodService(@Param("cod") String cod) ;
    ...
    @Query( "SELECT o FROM Elementos o WHERE id = :ids" )
    Elementos findByElementosIds1(@Param("ids") Long elemento_id);
}
