...
public interface HojaRutaRepository extends JpaRepository<HojaRuta, Long> {
    ...
    @Query(value = "SELECT TOP 1 h.* FROM hojaRuta h " +
            "INNER JOIN req_hoja_ruta rhdr ON h.id = rhdr.id_hoja_ruta " +
            "INNER JOIN requerimiento r ON r.id = rhdr.id_requerimiento " +
            "WHERE rhdr.id_estado <> 2 AND r.numero = :numReq", nativeQuery = true)
    HojaRuta findHdrByReqNumero(@Param("numReq") String numReq);
