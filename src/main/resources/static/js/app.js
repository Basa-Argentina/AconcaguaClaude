...
app.controller( 'AppCtrl',
    ...
    $scope.menuOptions = [
        ...
        null,
        ["Finalizar OK", function ($itemScope) {
            const row = $itemScope.selectedRow.entity;
        ...
        null,
        ["Finalizar ERROR", function ($itemScope) {
            const row = $itemScope.selectedRow.entity;
            row.fechaEntrega = new Date(row.fechaEntrega + "T" + row.horaEntrega + "-03:00");
            if(row.requerimiento.hojaRuta_id===null || row.requerimiento.hojaRuta.estado==="Anulada"){
                $http({
                    method: "POST",
                    url: "/siguiente_operacion/finalizar/saveError",
                    data: JSON.stringify(row),
                    headers: {"Content-Type": "application/json"}
                }).success(function () {
                    $scope.aplicarFiltro();
 ...
 * @param dataSet
  */
 $scope.enviarCorreo = function (dataSet) {
                        const rowToSend = [];
                        $scope.mostrarProgreso();
                        ...
                        if(dataSet instanceof Array) {
                            dataSet.forEach(function (row) {
                                if (row.tipoOperacion_id === "88" && row.requerimiento.remito_id) {
                                    rowToSend.push(row.id);
                                }
                            });
                        } else {
                            if (dataSet.tipoOperacion_id === "88" && dataSet.requerimiento.remito_id) {
                                rowToSend.push(dataSet.id);
...
app.directive('ngContextMenu', function () {
    const renderContextMenu = function ($scope, event, options) {
        $scope.selectedRow = angular.element(event.target).scope().row;
