                                                   ...
                                                   notifications)
{
    ...
    $scope.requerimientosTipo23 = [];

    $scope.armarListaRequerimientosTipo23 = function() {
        $scope.requerimientosTipo23 = [];

        // Asumiendo que $scope.$parent.row es un array de requerimientos
        for (var i = 0; i < $scope.$parent.row.length; i++) {
            var req = $scope.$parent.row[i];
            if (req && req.id_tipo_req === 23) {

                    $scope.requerimientosTipo23.push(req.num_req);

            }
        }
    };

    $scope.agregarElementosAutomaticamente = function() {
        for (var elemento in $scope.elementosList) {
            if ($scope.elementosList.hasOwnProperty(elemento)) {
                var req = $scope.elementosList[elemento].requerimiento;
                if (req && $scope.requerimientosTipo23.includes(req)) {
                    // Agrega automáticamente los elementos que cumplen con la condición
                    $scope.leidosList[elemento] = {
                        elemento: elemento,
                        requerimiento: req
                    };
                    delete $scope.elementosList[elemento];
                }
            }
        }
    };
     ...
     * @param keyEvent
     */
    $scope.armarListaRequerimientosTipo23();
    $scope.agregarElementosAutomaticamente();
