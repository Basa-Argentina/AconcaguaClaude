...
const printRemito = function (row, codigoEmpresa, nroRemito) {
    const codEmpresa = row.clienteEmp.empresaAdministra;
...
/*    row.sort((a, b) => {
        // Verificar si a.codigo_cliente y b.codigo_cliente son cadenas
        if (typeof a.codigo_cliente === 'string' && typeof b.codigo_cliente === 'string') {
            return a.codigo_cliente.localeCompare(b.codigo_cliente);
        } else {
            // Si no son cadenas, comparar directamente
...
const genTarea = function (posicion, detalle, item) {
...

    {
        ...
        if (item.referencias != null && item.referencias.length > 0 ) {
            hojaTarea.text(
...
const genCaja = function (item) {
  ...
  //  legajo.text(String(Number(item.codigo.substr(2,item.codigo.length))),2, 10);
    if (String(Number(item.codigo.substr(0,2))) === "13") {
        legajo.text(String(Number(item.codigo.substr(6,item.codigo.length))),50, 10);
    }else{
    legajo.text(String(Number(item.codigo.substr(2,item.codigo.length))),50, 10);
    }
    legajo.setFontSize(24);
 //   legajo.text(cliente, 60, 8);
    legajo.text(cliente, 2, 10);
