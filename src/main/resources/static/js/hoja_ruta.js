...
app.controller('HdRCtrl', function ($scope,
    ...
    //Labels buttons
    $scope.labelButtonImprimir   = 'Guardar';    //Sólo se muestra cuando la HdR se está creando
    ...
    $scope.gridOptionsHdR = {
        ...
        enableRowSelection             : true, // Habilitar la selección de fila
    ...
    $scope.setShowButtons = function(evaluaButtonControlar = true){ //Posible param "numero hoja ruta". 0 ó <> 0
        ...
        if ($scope.selectedHdR === '0'){    //Nueva HdR
            ...
            $scope.labelButtonReImprimir = ($scope.detectarCambiosEnGrilla() ? 'Nueva versión' : 'Imprimir');
  ...
  /*  $scope.imprimirPDF = function(pNumeroHdR, pCopia = 'ORIGINAL'){
    ...
    $scope.imprimirPDF = function(pNumeroHdR, pCopia = 'ORIGINAL'){
        ...
        // Obtener el orden actual de las filas en la grilla
        $scope.gridApiHdR.core.queueGridRefresh().then(function(){
            var rowOrder = $scope.gridApiHdR.core.getVisibleRows($scope.gridApiHdR.grid).map(function(row) {
                return row.entity; // Obtener los datos de la fila
            });

            // Pasar los datos en el orden actual a la función de impresión
            printHojaRuta(rowOrder, data, pCopia);
        });
    };
    ...
    $scope.guardarHdR = function(pNumeroHdR, pRequerimientos){
        ...
        // Continúa con la lógica de guardar los datos
        let defer = $q.defer();
