app.controller('marcarElementosCtrl', function ($scope,
    ...
    if (['2', '3', '4', '5', '6', '8', '10', '16', '18','23'].includes($scope.tipoRequerimientoId)) {
    ...
    $scope.agregarElemento = function (keyEvent) {
        ...
        if (keyPressedIn === 13) {
            ...
            if (!$scope.elementoLeido || $scope.elementoLeido.length === 0) {
                warnAlertSoundCleanFlag("No puede ingresar un elemento vacío");
            } else {
                if ((timestampOut - timestampIn) < 1500 || $scope.configAmbiente.ambiente === 'DEV') {
                    if ($scope.elementoLeido.length < 12) {
                        ...
                        $scope.checkReqelement($scope.elementoLeido,$scope.row.entity.id)
                            .then(result => {
                                if (result && result.codigo) {
                                    // Continuar con la lógica si el elemento es válido
                                    ...
                                    if ($element.length >= 1 && $scope.elementoLeido.length === 12) {
                                        if ($element.html().trim() == $scope.elementoLeido) {
                                            $scope.verificarElemento($scope.elementoLeido)
                                                .then(() => {
                                                    if ($scope.tipoRequerimientoId !== '23') {
                                                        if (elementoFactory.codigo === 'contenedor') {
                                                            warnAlert(`El elemento ${$scope.elemenctoLeido} ingresado posee legajos que no se encuentran "EN SALIDA". No puede continuar`, true);
                                                            $scope.disableAceptar = true;
                                                            $scope.disableDividir = true;
        ...
        } else if ((timestampOut - timestampIn) > 1500 && $scope.configAmbiente.ambiente === "PRD") {
            warnAlertSoundCleanFlag("No está permitido el ingreso manual. Debe utilizar el lector de código de barras");
        }
    };
    ...
    $scope.$on('evMarcarElemento', (event, data) => {
        ...
        }else if (['1','2', '3', '4', '5', '6','8', '16','23'].includes($scope.tipoRequerimientoId)) {
            $scope.disableAceptar = false;
            $scope.disableDividir = true;
        } else {
            $scope.disableAceptar = true;
    ...
    $scope.checkReqelement = function (codigo,operacion_id) {
        let url = `/requerimiento/${codigo}/${operacion_id}/verificar_Reqelemento`;
        ...
        return $http({
            ...
            const errorMessage = err.data && err.data.error ? err.data.error : "Error desconocido";
